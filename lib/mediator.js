...
module.exports = {
  unsubscribe: function (channel, fn, once, context) {
    var subscription = fn.toString() + !!once + channel + context
  ...
  subscribe: function (channel, fn, once, context) {
    ...
    var hash = mediator.generateHashCode(fn.toString() + !!once + channel + context)
    mediator.channels[channel].push({context: context, callback: fn, once: !!once})
    mediator.subscriptionHashes[hash] = mediator.channels[channel].length - 1
    return this
  },
