...
module.exports = {
  set: function (key, value) {
    ...
    events.publish('app', eventScope, key, value, {})
  },
  get: function (key) {
  ...
  onChange(key, fn, options) {
    if (fn instanceof Function) {
      events.subscribe('app', eventScope, key, fn, options)
    } else {
      throw new CakeException(null, true).throw('Wrong callback for onChange event with key %s', name)
    }
  },
  ignoreChange(key, fn, options) {
    if (fn instanceof Function) {
